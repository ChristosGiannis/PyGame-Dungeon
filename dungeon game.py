...
while not game.over:
    ...
    if story.collidedWith(mouse) and mouse.LeftButton:
        bk.draw()
        game.drawText("STORY:",85,90,f)
        game.drawText("YOU ARE A HERO THAT HAS BEEN PLACED IN A DUNGEON. YOUR GOAL",80,120,f)
        game.drawText("IS TO SLAY THE BOSS(the devil) INORDER TO ESCAPE.",80,150,f)
        game.drawText("YOU MUST PASS EACH FLOOR(LEVEL) OF THE DUNGEON TO PROGRESS.",80,180,f)
        game.drawText("TO THE NEXT. IN ORDER TO GO TO THE NEXT FLOOR, YOU MUST COLLECT,",85,210,f)
        game.drawText(" 3 PARTS OF A KEY UNTIL YOU REACH THE LAST FLOOR: THE BOSS FLOOR ",85,240,f)
        game.drawText("IN EVERY LEVEL THERE WILL BE A DIFFERENT OBSTACLE YOU FACE.",85,270,f)
        game.drawText("YOU WILL ENCOUNTER VILLAINS THAT KEEP YOU FROM TAKING THE KEYS ",85,300,f)
        game.drawText("Use the left/right arrow to move back and forth",85,350,f)
        game.drawText("Use the up arrow to jump",85,380,f)
...
while not game.over:
    ...
    bk.draw()
    hero.draw()
    game.drawText("LEVEL ONE",10,10)
    game.drawText("collect the three key parts to move to the next level",20,25)
    platform.draw()
    small.draw()
    med.draw()
    head.draw()
...
while not game.over:
    ...
    game.drawText("LEVEL TWO",10,10)
    game.drawText("avoid the fire but dont let the keys fall off the screen",20,25)
    platform.draw()
    hero.draw()
...
while not game.over:
    ...
    bk.draw()
    hero.draw()
    game.drawText("LEVEL THREE",10,10)
    game.drawText("avoid the DRAGONS",20,25)
    platform.draw()
    dragon.draw()
    small.draw()
    small2.draw()
